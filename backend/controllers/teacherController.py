...
async def delete_teacher(teacher_id: int, db: asyncpg.Connection):
    # Fix FUN-01: antes se borraba directo sin revisar dependencias. Ahora
    # se valida si el docente tiene ofertas de curso asignadas (mismo patron
    # usado en BG-U5 para estudiantes y en BG-C4 para ciclos/cursos).
    offerings_count = await db.fetchval(
        "SELECT COUNT(*) FROM course_offerings WHERE teacher_id = $1", teacher_id
    )
    if offerings_count and offerings_count > 0:
        return {
            "error": f"No se puede eliminar: el docente tiene {offerings_count} "
                     f"oferta(s) de curso asignada(s). Reasigna o elimina esas ofertas primero."
        }
